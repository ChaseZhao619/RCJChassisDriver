...
> 危险：本目录中的命令解析代码可以触发底盘移动、执行器动作、Flash 擦写和 MCU 复位。新增命令或改动参数校验时，必须保证非法参数、CRC 错误、超长帧和缺失参数不会触发危险动作。

使用者应在安全环境中测试所有会驱动硬件的命令。本项目不承担因错误命令、协议误用或上位机逻辑错误造成的硬件损坏和安全风险。

## 贡献与维护说明

> 重要：`App` 层直接影响外部通信协议。协议变更应优先保持向后兼容；如果必须破坏兼容性，必须在 README 中明确旧命令和新命令的差异。
